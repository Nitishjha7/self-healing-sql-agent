...
| `src/components/` | Sidebar, top bar, status bar, icons, SQL block, charts |
...
- **A status bar, where a feature strip used to be** — the bottom row used to list five claims: "Self-healing", "Safe by design", "Conversation memory", "Validation layer", "Observability". That is a landing page pinned inside an application. It said the same five things whether the agent was idle, mid-retry, or unable to reach Postgres, so it carried no information — and two of the five could be false at runtime, because memory switches itself off when the checkpointer cannot start. It is now a real status bar fed by `/api/meta`: model, Postgres version, direct-vs-MCP transport, retry budget, whether an approved write commits or rolls back, and whether memory is on — plus what the last run actually cost. **Every field is read from the running process, not typed into the component.** It caught its own first bug immediately: the running model was `gemini-3.1-flash-lite` while every doc and default said `3.5`.
- **One viewport, two scroll panes** — the shell is `100dvh` and never scrolls itself; the content column and the trace rail each scroll on their own. Previously the whole document scrolled, which meant the status bar sat at the end of the flow and was visible only on a short conversation. A bar that reports what the agent is doing is useless at the bottom of a scroll nobody is at. This is also what let the trace rail drop its `position: sticky` and `calc(100vh - 120px)` cap — both were workarounds for the page-level scroll.
- **The header no longer greets anyone** — it used to open with "Good morning, Nitish 👋". There is no authentication in this app, so it never knew who was looking at it; that was decoration dressed as personalisation. The same header hard-coded "PostgreSQL 18" as a pill, which would have kept saying 18 against any other server. Both facts moved to the status bar and are now asked of the backend.
